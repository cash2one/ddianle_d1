...
-------------------以下是一个礼包的定义
--礼品包的详细信息
function GetPacketItemsInfo_34130(RoleIndex, PacketID)
	local Sex = GetRoleSex(RoleIndex)
	if Sex == SexType_Male then
		return "7003      ,1   ,2592000,9011      ,1   ,2592000,5006      ,1   ,-1"		--物品1ID 数量 有效时间1, 物品2ID 数量2 有效时间2, 物品ID3 数量3 有效时间3
	else
		return "7003      ,1   ,2592000,9011      ,1   ,2592000,5006     ,1   ,-1"		--物品1ID 数量 有效时间1, 物品2ID 数量2 有效时间2, 物品ID3 数量3 有效时间3
	end
end
...
-- 礼包定义,需要填物品ID号,数量
function PacketGetReward_34130 (RoleIndex)
	local nPacketID = 34130
	local Sex = GetRoleSex(RoleIndex)
	--礼包的道具数量,必填
	local Count = 3
	local ItemTable = {}
...
--注册获取礼包信息函数
-- 礼包ID 礼包描述函数
RegisterGetPacketInfo(34130, GetPacketItemsInfo_34130)
--注册礼包使用函数
--                    礼包ID 礼包获取函数
RegisterGetPacketItem(34130, PacketGetReward_34130)
